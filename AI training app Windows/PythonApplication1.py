...
def TrainAI(features, labels):
    ...
    network.add(Conv2D(128,(3,3), input_shape=(features_train.shape[1],features_train.shape[2],features_train.shape[3],), name='cnnNet00'))
    network.add(MaxPooling2D(pool_size=(2,2)))
    #network.add(Normalization())
    network.add(Conv2D(512,(3,3), name='cnnNet0'))
    network.add(MaxPooling2D(pool_size=(2,2)))
   ...
   # network.add(MaxPooling2D(pool_size=(1,1)))
    ...
    network.add(MaxPooling2D(pool_size=(2,2)))
    network.add(Conv2D(2048,(3,3), activation="relu", name='cnnNet5'))
   # network.add(MaxPooling2D(pool_size=(2,2)))
    network.add(GlobalAveragePooling2D())

    #network.add(Flatten())
    network.add(Dense(4096, activation="relu", name="layerDense1"))
    #network.add(Dropout(0.05))
    ...
    network.fit(features_train, labels_train, validation_data=(features_val,labels_val),epochs = epochs, batch_size=2, verbose=2, callbacks=TqdmCallback(verbose=1))
    time.sleep(5)
